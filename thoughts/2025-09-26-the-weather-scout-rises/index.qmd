...
[The Weather Scout](https://theweatherscout.com) is an interactive site that allows users to scout typical weather conditions across the US based on 30-year climate normal data and models. I recently rebuilt the site using [Amp](https://ampcode.com/) (Sourcegraph's agentic coding tool), and this post details the process of the build. One interesting feature of [Amp](https://ampcode.com/) is that each thread (conversation) is saved on the server and can be shared easily. Throughout this post, I will include some of my actual threads so you can see how I prompted the models and utilized [Amp](https://ampcode.com/) during the build.

My biggest ah-ha moments were:

1. [Architecture advice, exploration and problem solving with the oracle](#architecture-advice)
2. [Troubleshooting with Playwright MCP](#troubleshooting-with-playwright-mcp)
3. [UI overhaul from a Figma sketch](#ui-enhancements)

![The Weather Scout homepage](weather-scout-homepage.png){#fig-weather-scout-homepage}
...
![Amp provides TUI running in my terminal, VS Code extension also available](amp-terminal.png){#fig-amp-terminal fig-align="left" width=800}
...
![Architecture back and forth example](architecture-example.png){#fig-architecture fig-align="left" width=800}
...
After lots of preparation, it was time to make The Weather Scout finally come alive. I prompted [Amp](https://ampcode.com/) to execute each phase in `BUILD_TODO.md` one by one. I find it easier to keep the context window under control and review changes when I instruct the agent to execute sections at a time. Next, I noticed some VS Code IDE diagnostic errors in the TypeScript codebase, so I started a fresh thread and had [Amp](https://ampcode.com/) quickly resolve those issues. [Amp](https://ampcode.com/) can retrieve diagnostic information from the IDE, which makes it efficient at identifying and fixing errors like this. I was able to run the frontend and workers locally for the first time and visit the URL in my browser at a localhost address, exciting!

![Executing a plan section by section in separate threads to manage context](execute-plan-example.png){#fig-execute-plan fig-align="left" width=800}
...
![Mapbox search finding the right location fast](search-example.png){#fig-mapbox-search fig-align="left" width=400}
...
![Original vs desired state sketched in Figma](old_to_new_ui.png){#fig-ui-transform}

![New charts Amp created from the sketch](ui-after.png){#fig-new-charts fig-align="left" width=800}


### Deployment

Deployment was pretty straight forward because I spent so much time upfront with [Amp](https://ampcode.com/) planning the Architecture and integrating services from the beginning, I already knew how services would be hosted on Cloudflare, and the raster data was already in R2 buckets all along. I had [Amp](https://ampcode.com/) help me create a checklist (with the Oracle's help, of course) of items to prepare for deployment, including Cloudflare configuration, CI/CD pipelines, a release strategy, and additional security/cost implementations.
...
You can check out the website live at [https://theweatherscout.com](https://theweatherscout.com)
I hope you learned something from my journey, feel free to reach out [on X](https://x.com/tnederlof) or email me at [trevor.nederlof@hey.com](mailto:trevor.nederlof@hey.com)

## Learnings

Taking on a project from start to finish using [Amp](https://ampcode.com/) was a rewarding experience. Using these agentic tools is a skill that needs to be cultivated. We are only in the early innings and I am eager to continue building. My biggest takeaways are to start new threads more often, slow down and make deliberate architectural decisions, and to have the models check their own work continuously. Sometimes the models produce precisely what you are looking for, and sometimes they produce terrible code. It's a highly irregular, you always need to be on your toes and supervising. Still, I created this site in significantly less time than if I were coding by hand and learned along the way.